...
/**
 * Seems the <code>com.amazonaws.services.glacier.TreeHashGenerator</code> class
 * from <a href=
 * "https://docs.aws.amazon.com/amazonglacier/latest/dev/uploading-an-archive-mpu-using-java.html">AWS's
 * multi-part upload example</a> is no longer in the SDK. Fortunately they have
 * some other <a href=
 * "https://docs.aws.amazon.com/amazonglacier/latest/dev/checksum-calculations.html#checksum-calculations-examples">tree-hash
 * example code<a/>, on which this class is based.
 */
public class TreeHashUtil {
	 ...
	 * @return a byte[][] containing the checksums of each <code>partLength</code> chunk
	 * @throws IOException
	 *             Thrown if there's an IOException when reading the file
	 * @throws NoSuchAlgorithmException
	 *             Thrown if SHA-256 MessageDigest can't be found
	 * @throws IllegalArgumentException
	 *             Thrown if <code>partLength</code> is not a power of 2.
	 */
	 ...
	 * 
	 * @param byteRange
	 *            A range of bytes to compute checksums on
	 * @return a byte[][] containing the checksums of each 1 MB portion of byteRange
	 * @throws IOException
	 *             Thrown if there's an IOException when reading the file
	 * @throws NoSuchAlgorithmException
	 *             Thrown if SHA-256 MessageDigest can't be found
	 ...
	 *             Thrown if <code>partLength</code> is not a power of 2.
	 */
	public static byte[][] getChunkSHA256Hashes(byte[] byteRange) throws IOException, NoSuchAlgorithmException {		
		MessageDigest md = MessageDigest.getInstance("SHA-256");

		long numChunks = byteRange.length / ONE_MB;
		if (byteRange.length % ONE_MB > 0) {
			numChunks++;
		}
		...
		byte[] buff = new byte[ONE_MB];
		try (ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(byteRange)) {
			int bytesRead;
			int idx = 0;
			
			while ((bytesRead = byteArrayInputStream.read(buff, 0, ONE_MB)) > 0) {
				md.reset();
				md.update(buff, 0, bytesRead);
				chunkSHA256Hashes[idx++] = md.digest();
			}
	 ...
	 * 
	 * This method uses a pair of arrays to iteratively compute the tree hash level
	 * by level. Each iteration takes two adjacent elements from the previous level
	 * source array, computes the SHA-256 hash on their concatenated value and
	 * places the result in the next level's destination array. At the end of an
	 * iteration, the destination array becomes the source array for the next level.
	 * 
